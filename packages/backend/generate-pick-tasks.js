  ...
  try {
    ...
    for (const order of result.rows) {
      ...
      for (const item of itemsResult.rows) {
        ...
        `, [pickTaskId, order.order_id, item.order_item_id, item.sku, item.name, item.bin_location, item.quantity, 0]);
        
        console.log(`    Created task ${pickTaskId}: ${item.sku} x${item.quantity} at ${item.bin_location}`);
        totalTasks++;
      }
    }
