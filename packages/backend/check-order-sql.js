...
async function checkOrder(orderId) {
  try {
    ...
    itemsResult.rows.forEach((item, i) => {
      console.log(`  ${i + 1}. ${item.sku} - ${item.picked_quantity}/${item.quantity} (${item.status})`);
    });
    console.log();
    ...
    console.log(`   Current status: ${order.status} - ${order.status === 'PENDING' || order.status === 'PICKING' || order.status === 'IN_PROGRESS' ? '✅ PASS' : '❌ FAIL'}`);
    console.log();
    console.log('2. Has items in order_items table');
    console.log(`   Item count: ${itemsResult.rowCount} - ${itemsResult.rowCount > 0 ? '✅ PASS' : '❌ FAIL'}`);
    console.log();
    console.log('3. Has pick tasks in pick_tasks table');
    console.log(`   Task count: ${tasksResult.rowCount} - ${tasksResult.rowCount > 0 ? '✅ PASS' : '❌ FAIL'}`);
    console.log();
    console.log('4. Progress < 100');
    console.log(`   Progress: ${order.progress}% - ${order.progress < 100 ? '✅ PASS' : '❌ FAIL'}`);
    console.log();
    
    const allPass = (order.status === 'PENDING' || order.status === 'PICKING' || order.status === 'IN_PROGRESS') && 
