  ...
  try {
    ...
    if (stuckPickersResult.rows.length > 0) {
      stuckPickersResult.rows.forEach(row => {
        console.log(
          `  - ${row.name} (${row.user_id}): ${row.stuck_orders} orders, last activity ${row.last_activity}`
        );
      });
    } else {
    ...
    for (const picker of stuckPickersResult.rows) {
      ...
      ordersResult.rows.forEach(row => {
        console.log(
          `  - Order ${row.order_id}: ${row.customer_name}, ${row.progress}% complete, updated ${row.updated_at}`
        );
      });
    }
    console.log();
    ...
    for (const picker of stuckPickersResult.rows) {
      ...
      await query(`DROP TRIGGER IF EXISTS trigger_log_order_state_change ON orders`);
      ...
      await query(`UPDATE users SET current_task_id = NULL WHERE user_id = $1`, [picker.user_id]);
