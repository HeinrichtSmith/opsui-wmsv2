...
async function checkPickerData() {
  try {
    ...
    const users = await pool.query('SELECT user_id, name, email, role, active FROM users WHERE role = \'PICKER\' ORDER BY user_id');
    ...
    const orders = await pool.query('SELECT order_id, picker_id, status, progress, updated_at FROM orders WHERE picker_id IS NOT NULL ORDER BY updated_at DESC LIMIT 10');
    ...
    console.log('\n=== CHECKING PICK TASKS ===');
    const tasks = await pool.query('SELECT pick_task_id, picker_id, order_id, started_at, completed_at, status FROM pick_tasks WHERE picker_id IS NOT NULL ORDER BY started_at DESC LIMIT 10');
