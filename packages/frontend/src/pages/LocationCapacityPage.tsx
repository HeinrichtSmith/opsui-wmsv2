...
function CapacityBar({ utilization, maximum, warningThreshold }: { utilization: number; maximum: number; warningThreshold: number }) {
  ...
  return (
    <div className="relative">
      ...
      <div className="flex justify-between mt-1 text-xs text-gray-500">
        <span>{utilization.toFixed(1)} / {maximum} {utilization > maximum ? '⚠' : ''}</span>
...
function CapacityRuleModal({ rule, onClose, onSuccess }: { rule?: any; onClose: () => void; onSuccess: () => void }) {
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => setFormData({ ...formData, capacityType: e.target.value as CapacityType })}
                className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
              >
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => setFormData({ ...formData, capacityUnit: e.target.value as CapacityUnit })}
                className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
              >
            ...
            {(formData.appliesTo === 'ZONE' || formData.appliesTo === 'LOCATION_TYPE' || formData.appliesTo === 'SPECIFIC_LOCATION') && (
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">
                ...
                {formData.appliesTo === 'ZONE' ? (
                  ...
                  <select
                    ...
                    onChange={(e) => setFormData({ ...formData, locationType: e.target.value as BinType })}
                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
                  >
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => setFormData({ ...formData, maximumCapacity: parseFloat(e.target.value) })}
                className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
              />
            </div>
            ...
            {formData.allowOverfill && (
              <div>
                ...
                <input
                  ...
                  onChange={(e) => setFormData({ ...formData, overfillThreshold: parseFloat(e.target.value) })}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500"
                />
              </div>
            )}
...
export function LocationCapacityPage() {
  ...
  const canManageRules = user?.role === UserRole.STOCK_CONTROLLER || user?.role === UserRole.SUPERVISOR || user?.role === UserRole.ADMIN;
        ...
        <div className="space-y-6">
          ...
          <div className="flex justify-between items-center">
            <div>
              ...
              <p className="text-gray-400 mt-1">Monitor and manage bin location capacity constraints</p>
            </div>
        {canManageRules && (
          <button
            onClick={() => {
          ...
          <div className="p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-6">
              ...
              <div className="bg-green-900/30 border border-green-800 rounded-lg p-4">
                ...
                <div className="text-2xl font-bold text-green-100 mt-1">{totalCapacity.toFixed(0)}</div>
              </div>
              <div className="bg-yellow-900/30 border border-yellow-800 rounded-lg p-4">
                ...
                <div className="text-2xl font-bold text-yellow-100 mt-1">{overallUtilization.toFixed(1)}%</div>
              </div>
            ...
            <div className="mb-6">
              <h3 className="text-lg font-semibold text-white mb-3">Overall Capacity Utilization</h3>
              <CapacityBar
                    ...
                    {capacities.map((capacity: any) => (
                      <tr key={capacity.capacityId} className="hover:bg-gray-800/50">
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="text-sm font-medium text-white">{capacity.binLocation}</div>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
              ...
              <table className="min-w-full divide-y divide-gray-800">
                <thead className="bg-gray-900/50">
                  <tr>
                    ...
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-400 uppercase">Type</th>
                    <th className="px-6 py-3 text-left text-xs font-medium text-gray-400 uppercase">Applies To</th>
                    <th className="px-6 py-3 text-right text-xs font-medium text-gray-400 uppercase">Max Capacity</th>
                    <th className="px-6 py-3 text-right text-xs font-medium text-gray-400 uppercase">Warning %</th>
                    <th className="px-6 py-3 text-center text-xs font-medium text-gray-400 uppercase">Priority</th>
                    <th className="px-6 py-3 text-right text-xs font-medium text-gray-400 uppercase">Actions</th>
                  </tr>
                </thead>
                <tbody className="bg-gray-900/30 divide-y divide-gray-800">
                      ...
                      <tr key={rule.ruleId} className="hover:bg-gray-800/50">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <div className="text-sm text-white">
                            ...
                            {rule.appliesTo === 'SPECIFIC_LOCATION' && `Location: ${rule.specificLocation}`}
                          </div>
                        </td>
                      ...
                      <div className="flex items-start">
                        ...
                        <div>
                          <h4 className={`font-semibold ${
                            ...
                            {alert.binLocation} - {alert.capacityType} Capacity {alert.alertType}
                          </h4>
                          <p className="text-sm mt-1 text-gray-300">{alert.alertMessage}</p>
                          <div className="text-sm mt-2 text-gray-300">
                            <span className="font-medium text-white">Current: </span>
                            {alert.currentUtilization.toFixed(1)} / {alert.maximumCapacity} ({alert.utilizationPercent.toFixed(1)}%)
                          </div>
                        </div>
                      </div>
                      <button
