...
import { useDashboardMetrics, usePickerActivity, usePickerOrders, usePackerActivity, usePackerOrders, useStockControllerActivity, useStockControllerTransactions } from '@/services/api';
      ...
      <CardContent className="p-6">
        <div className="flex items-center justify-between">
          <div className="flex-1">
            <p className="text-sm font-medium dark:text-gray-400 text-gray-500 uppercase tracking-wider">{title}</p>
            <p className="mt-3 text-4xl font-bold dark:text-white text-gray-900 tracking-tight group-hover:scale-105 transition-transform duration-300">{value}</p>
          </div>
...
export function DashboardPage() {
  ...
  const [selectedController, setSelectedController] = useState<{ id: string; name: string } | null>(null);
  ...
  const { data: controllerTransactions, isLoading: controllerTransactionsLoading } = useStockControllerTransactions(
    selectedController?.id || '',
    !!selectedController
  );

  // Fetch stock controller activity
  const { data: stockControllerActivity, isLoading: stockControllerActivityLoading } = useStockControllerActivity({
    enabled: canSupervise()
  });
  ...
  const handleViewOrder = (orderId: string) => {
    ...
    if (isPackerOrder) {
      ...
      console.log('[Dashboard] No context, defaulting to packing page:', `/packing/${orderId}/pack`);
      navigate(`/packing/${orderId}/pack`);
    }
  };
  ...
  return (
    <div className="min-h-screen">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-8 space-y-6 sm:space-y-8">
        ...
        <h1 className="text-2xl sm:text-3xl font-bold dark:text-white text-gray-900 tracking-tight">Dashboard</h1>
        <p className="mt-2 dark:text-gray-400 text-gray-600 text-responsive-sm">Real-time warehouse operations overview</p>
      </div>
        ...
        <CardContent>
          <div className="grid grid-cols-3 gap-4 sm:gap-6">
            ...
            <div className="text-center p-3 sm:p-4 rounded-xl dark:bg-white/[0.02] bg-gray-50 dark:border-white/[0.05] border-gray-200">
              ...
              <p className="text-xs sm:text-sm dark:text-gray-400 text-gray-600 mt-1">This Week</p>
            </div>
            <div className="text-center p-3 sm:p-4 rounded-xl dark:bg-white/[0.02] bg-gray-50 dark:border-white/[0.05] border-gray-200">
              <p className="text-2xl sm:text-3xl font-bold dark:text-white text-gray-900 tracking-tight">
                {metrics.throughput.month}
              </p>
              <p className="text-xs sm:text-sm dark:text-gray-400 text-gray-600 mt-1">This Month</p>
            </div>
          </div>
        </CardContent>
      </Card>
              ...
              <table className="min-w-full dark:divide-y dark:divide-white/[0.08] divide-y divide-gray-200">
                <thead>
                  <tr>
                    ...
                    {pickerActivity && Array.isArray(pickerActivity) && pickerActivity.some((p: any) => p.currentOrderId) && (
                      <th className="px-2 sm:px-4 py-3 text-left text-xs font-semibold dark:text-gray-400 text-gray-600 uppercase tracking-wider">
                        Progress
                      </th>
                    )}
                ...
                <tbody className="dark:divide-y dark:divide-white/[0.05] divide-y divide-gray-200">
                      ...
                      console.log('[Dashboard] Processing picker:', picker.pickerName, 'currentView:', picker.currentView, 'status:', status);

                      if (picker.currentView) {
                        // Check for order queue - handle both "Order Queue" text and "/orders" path
                        if (picker.currentView === 'Order Queue' ||
                        ...
                        else if (picker.currentView.includes('Picking Order') ||
                        ...
                        else if (picker.currentView.includes('Profile') || picker.currentView.includes('/profile')) {
                          displayView = 'Profile';
                      ...
                      console.log('[Dashboard] Display view:', displayView, 'Label:', viewLabel, 'Status:', status);
                      ...
                      return (
                        <tr key={`picker-${pickerId}`} className="dark:hover:bg-white/[0.02] hover:bg-gray-50 transition-colors">
                        <td className="px-2 sm:px-4 py-3 text-sm dark:text-white text-gray-900">
                          <div className="flex flex-col">
                            ...
                            <span className="text-xs dark:text-gray-500 text-gray-500">({picker.pickerId})</span>
                          </div>
                        </td>
                        <td className="px-2 sm:px-4 py-3 text-sm">
                          <Button
                            size="sm"
                            variant="secondary"
                            onClick={() => setSelectedPicker({ id: picker.pickerId, name: picker.pickerName })}
              ...
              <table className="min-w-full dark:divide-y dark:divide-white/[0.08] divide-y divide-gray-200">
                <thead>
                  <tr>
                    ...
                    {packerActivity && Array.isArray(packerActivity) && packerActivity.some((p: any) => p.currentOrderId) && (
                      <th className="px-4 py-3 text-left text-xs font-semibold dark:text-gray-400 text-gray-600 uppercase tracking-wider">
                        Progress
                      </th>
                    )}
                ...
                <tbody className="dark:divide-y dark:divide-white/[0.05] divide-y divide-gray-200">
                      ...
                      console.log('[Dashboard] Processing packer:', packer.packerName, 'currentView:', packer.currentView, 'status:', status);

                      if (packer.currentView) {
                        // Check for packing queue - handle both "Packing Queue" text and "/packing" path
                        if (packer.currentView === 'Packing Queue' ||
                        ...
                        else if (packer.currentView.includes('Packing Order') ||
                        ...
                        // Check for profile page
                        else if (packer.currentView.includes('Profile') || packer.currentView.includes('/profile')) {
                          displayView = 'Profile';
                          viewLabel = 'badge-info';
                        }
                      ...
                      return (
                        <tr key={`packer-${packerId}`} className="dark:hover:bg-white/[0.02] hover:bg-gray-50 transition-colors">
                        <td className="px-4 py-3 text-sm dark:text-white text-gray-900">
                          {packer.packerName} <span className="text-xs dark:text-gray-500 text-gray-500">({packer.packerId})</span>
                        </td>
                        <td className="px-4 py-3 text-sm">
                          <Button
                            size="sm"
                            variant="secondary"
                            onClick={() => setSelectedPacker({ id: packer.packerId, name: packer.packerName })}
                            className="text-xs"
                          >
                            <DocumentTextIcon className="h-3 w-3 mr-1" />
                            View Orders
      ...
      <Card variant="glass" className="card-hover">
        ...
        <CardContent>
          {stockControllerActivityLoading ? (
            <div className="flex items-center justify-center py-12">
              <div className="dark:text-gray-400 text-gray-500">Loading stock controller activity...</div>
            </div>
          ) : stockControllerActivity && Array.isArray(stockControllerActivity) && stockControllerActivity.length > 0 ? (
            <div className="overflow-x-auto">
              <table className="min-w-full dark:divide-y dark:divide-white/[0.08] divide-y divide-gray-200">
                <thead>
                  <tr>
                ...
                <tbody className="dark:divide-y dark:divide-white/[0.05] divide-y divide-gray-200">
                  {stockControllerActivity?.map((controller: any, idx) => {
                    ...
                    console.log('[Dashboard] Processing stock controller:', controller.controllerName, 'currentView:', controller.currentView, 'status:', status);

                    if (controller.currentView) {
                      // Check for stock control dashboard
                      if (controller.currentView === 'Stock Control Dashboard' ||
                      ...
                      else if (controller.currentView === 'Stock Control - Inventory' ||
                      ...
                      else if (controller.currentView === 'Stock Control - Transactions' ||
                      ...
                      else if (controller.currentView === 'Stock Control - Quick Actions' ||
                               controller.currentView.includes('Quick Actions')) {
                        displayView = 'Quick Actions';
                        viewLabel = 'badge-warning';
                      }
                      // Check for profile page
                      else if (controller.currentView.includes('Profile') || controller.currentView.includes('/profile')) {
                        displayView = 'Profile';
                        viewLabel = 'badge-info';
                      }
                      // Default: use view name as-is
                    ...
                    return (
                      <tr key={`controller-${controllerId}`} className="dark:hover:bg-white/[0.02] hover:bg-gray-50 transition-colors">
                        <td className="px-4 py-3 text-sm dark:text-white text-gray-900">
                          {controller.controllerName} <span className="text-xs dark:text-gray-500 text-gray-500">({controller.controllerId})</span>
                        </td>
                        <td className="px-4 py-3 text-sm">
                          <Button
                            size="sm"
                            variant="secondary"
                            onClick={() => setSelectedController({ id: controller.controllerId, name: controller.controllerName })}
                            className="text-xs"
                          >
            ...
            <div className="flex flex-col items-center justify-center py-16">
              ...
              <p className="text-sm dark:text-gray-400 text-gray-600">No active stock controllers</p>
              <p className="text-xs dark:text-gray-500 text-gray-500 mt-2">
                    ...
                    {pickerOrders
                      ...
                      .map((order: any) => {
                        ...
                        const selectedPickerActivity = Array.isArray(pickerActivity)
                          ? pickerActivity.find((p: any) => p.pickerId === selectedPicker?.id)
                          : undefined;
                        const isCurrentlyPicking = selectedPickerActivity?.currentOrderId === order.orderId;
                        ...
                        <div className="flex items-start justify-between">
                          <div className="flex-1">
                            <div className="flex items-center gap-2">
                              <h4 className="text-sm font-semibold dark:text-white text-gray-900">{order.orderId}</h4>
                              ...
                              {!isCurrentlyPicking && (order.status === 'PICKED' || order.progress === 100) && (
                            ...
                            <p className="text-sm dark:text-gray-300 text-gray-600 mt-1">{order.customerName}</p>
                            <div className="mt-2 grid grid-cols-3 gap-4 text-xs dark:text-gray-400 text-gray-600">
                              <div>
                                <span className="font-medium dark:text-white text-gray-900">Progress:</span> {order.progress}%
                              </div>
                              <div>
                                <span className="font-medium dark:text-white text-gray-900">Items:</span> {order.itemCount || 0}
                              </div>
                              <div>
                                <span className="font-medium dark:text-white text-gray-900">Priority:</span> {order.priority}
                              </div>
                            </div>
                            ...
                            {order.items && order.items.length > 0 && (
                              <div className="mt-3 space-y-2">
                                <p className="text-xs font-medium dark:text-gray-300 text-gray-600">Items to Pick:</p>
                                <div className="space-y-1">
                                  {order.items.map((item: any, itemIdx: number) => {
                                    const itemStatusColor = item.status === 'FULLY_PICKED'
                    ...
                    {packerOrders
                      ...
                      .map((order: any) => {
                        ...
                        const selectedPackerActivity = Array.isArray(packerActivity)
                          ? packerActivity.find((p: any) => p.packerId === selectedPacker?.id)
                          : undefined;
                        const isCurrentlyPacking = selectedPackerActivity?.currentOrderId === order.orderId;

                        return (
                      <div
                        key={order.orderId}
                        onClick={() => {
                          console.log('[Dashboard] Packer order card clicked:', order.orderId);
                          handleViewOrder(order.orderId);
                        }}
                        className="dark:border dark:border-white/[0.08] border border-gray-200 rounded-xl p-4 dark:hover:bg-primary-500/10 hover:bg-primary-50 dark:hover:border-primary-500/30 hover:border-primary-500/30 cursor-pointer transition-all duration-300 hover:scale-[1.02]"
                      >
            ...
            <div className="inline-block align-bottom glass-card rounded-2xl text-left overflow-hidden shadow-premium-lg transform transition-all sm:my-8 sm:align-middle sm:max-w-2xl sm:w-full animate-scale-in">
              ...
              <div className="dark:bg-white/[0.02] bg-gray-50 px-6 py-4 sm:px-6 flex items-center justify-between dark:border-b border-b dark:border-white/[0.08] border-gray-200">
                <div>
                  ...
                  <p className="mt-1 text-sm dark:text-gray-400 text-gray-600">
                    {controllerTransactions?.length || 0} transaction{controllerTransactions?.length !== 1 ? 's' : ''}
                  </p>
                </div>
                <button
                  onClick={() => setSelectedController(null)}
              ...
              <div className="px-6 py-5 sm:px-6 max-h-96 overflow-y-auto">
                {controllerTransactionsLoading ? (
                  <div className="flex items-center justify-center py-12">
                    <div className="dark:text-gray-400 text-gray-500">Loading transactions...</div>
                  </div>
                          ...
                          <div className="flex items-start justify-between">
                            <div className="flex-1">
                              <div className="flex items-center gap-2 mb-2">
                                ...
                                <h4 className="text-sm font-semibold dark:text-white text-gray-900">{transaction.sku}</h4>
                              </div>
                              <div className="grid grid-cols-2 gap-4 text-xs dark:text-gray-400 text-gray-600">
                                <div>
                                  <span className="font-medium dark:text-white text-gray-900">Bin:</span> {transaction.binLocation || 'N/A'}
                                </div>
                                <div>
                                  <span className="font-medium dark:text-white text-gray-900">Quantity:</span> {transaction.quantityChange > 0 ? `+${transaction.quantityChange}` : transaction.quantityChange}
                                </div>
                                <div className="col-span-2">
                                  <span className="font-medium dark:text-white text-gray-900">Reason:</span> {transaction.reason || 'N/A'}
                                </div>
                                <div className="col-span-2">
                                  <span className="font-medium dark:text-white text-gray-900">Time:</span> {timestamp}
                                </div>
                              </div>
                            </div>
                          </div>
                  ...
                  <div className="text-center py-12">
                    ...
                    <p className="text-sm dark:text-gray-400 text-gray-600">No transactions found</p>
                    <p className="text-xs dark:text-gray-500 text-gray-500 mt-2">
