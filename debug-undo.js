...
(async () => {
  try {
    ...
    const orderResult = await query(`SELECT order_id FROM orders WHERE status = 'PICKING' LIMIT 1`);
    ...
    const orderBefore = await query(`SELECT progress FROM orders WHERE order_id = $1`, [orderId]);
    ...
    if (itemsBefore.rows.length > 0) {
      ...
      const orderAfter = await query(`SELECT progress FROM orders WHERE order_id = $1`, [orderId]);
      ...
      console.log(`\n=== RESULT ===`);
      console.log(
        `Progress changed: ${orderBefore.rows[0].progress !== orderAfter.rows[0].progress ? 'YES' : 'NO'}`
      );
      ...
      if (orderBefore.rows[0].progress === orderAfter.rows[0].progress) {
        ...
        console.log(
          `✓ Trigger fired! Progress changed from ${orderBefore.rows[0].progress}% to ${orderAfter.rows[0].progress}%`
        );
      }
    }
